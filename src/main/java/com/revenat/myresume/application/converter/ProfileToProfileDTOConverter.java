	...
	private List<HobbyDTO> getHobbies(Profile profile) {
		...
		if (hobbies != null) {
			for (Hobby hobby : hobbies) {
				HobbyDTO dto = new HobbyDTO(hobby, profile.getId());
				dtoList.add(dto);
			}
		}
		return dtoList;
	...
	private List<SkillDTO> getSkills(Profile profile) {
		...
		if (skills != null) {
			for (Skill skill : skills) {
				SkillDTO dto = new SkillDTO(skill, profile.getId());
				dtoList.add(dto);
			}
		}
		return dtoList;
	...
	private List<LanguageDTO> getLanguages(Profile profile) {
		...
		if (languages != null) {
			for (Language language : languages) {
				LanguageDTO dto = new LanguageDTO(language, profile.getId());
				dtoList.add(dto);
			}
		}
		return dtoList;
	...
	private List<ExperienceDTO> getExperience(Profile profile) {
		...
		if (exp != null) {
			for (Experience experience : exp) {
				ExperienceDTO dto = new ExperienceDTO(experience, profile.getId());
				dtoList.add(dto);
			} 
		}
		return dtoList;
	...
	private List<EducationDTO> getEducations(Profile profile) {
		...
		if (educations != null) {
			for (Education education : educations) {
				EducationDTO dto = new EducationDTO(education, profile.getId());
				dtoList.add(dto);
			} 
		}
		return dtoList;
	...
	private List<CourseDTO> getCourses(Profile profile) {
		...
		if (courses != null) {
			for (Course course : courses) {
				CourseDTO dto = new CourseDTO(course, profile.getId());
				dtoList.add(dto);
			}
		}
		return dtoList;
	...
	private List<CertificateDTO> getCertificates(Profile profile) {
		...
		if (certificates != null) {
			for (Certificate certificate : certificates) {
				CertificateDTO dto = new CertificateDTO(certificate, profile.getId());				
				dtoList.add(dto);
			}
		}
		return dtoList;
